...
varscan2seqz <- function (varscan.somatic, varscan.copynumber = NULL, normal_var_freq = 0.25){
  ...
  varscan.somatic$normal_var_freq <- as.numeric(sub("%", "", 
                                                    varscan.somatic$normal_var_freq))/100
  varscan.somatic$tumor_var_freq <- as.numeric(sub("%", "", 
                                                   varscan.somatic$tumor_var_freq))/100
  ...
  if (sum(idx) > 0) {
    ...
    mut.b <- sapply(X = 1:sum(idx), FUN = function(x) gsub(x = mut.b[x, 
                                                                     1], pattern = mut.b[x, 2], replacement = ""))
    mut <- paste0(mut.b, varscan.somatic$tumor_var_freq[idx])
    strand[idx] <- paste0(mut.b, varscan.somatic$tumor_reads2_plus[idx]/(varscan.somatic$tumor_reads2_plus[idx] + 
                                                                           varscan.somatic$tumor_reads2_minus[idx]))
    AB.tumor[idx] <- mut
  }
  res <- data.frame(chromosome = as.character(varscan.somatic$chrom), 
                    position = varscan.somatic$position, base.ref = as.character(varscan.somatic$ref), 
                    depth.normal = depth.normal, depth.tumor = depth.tumor, 
                    depth.ratio = depth.ratio, Af = round(Af, 3), Bf = round(Bf, 
                                                                             3), zygosity.normal = zygosity.normal, GC.percent = 50, 
  ...
  normal.pos <- res$zygosity.normal == "hom" & res$AB.tumor == 
    "."
  res <- res[res$depth.ratio > 0 & !is.infinite(res$depth.ratio) & 
               !normal.pos, ]
  if (!is.null(varscan.copynumber)) {
    smart.id <- order(c(1:nrow(varscan.copynumber), 1:nrow(varscan.copynumber) + 
                          0.5))
    varscan.copynumber$log2_ratio <- 2^(varscan.copynumber$log2_ratio)
    varscan.copynumber$normal_depth <- round(varscan.copynumber$normal_depth, 
                                             0)
    varscan.copynumber$tumor_depth <- round(varscan.copynumber$tumor_depth, 
                                            0)
    varscan.copynumber$chrom <- as.character(varscan.copynumber$chrom)
    mat.t <- data.frame(chromosome = c(varscan.copynumber$chrom, 
                                       varscan.copynumber$chrom)[smart.id], position = c(varscan.copynumber$chr_start, 
                                                                                         varscan.copynumber$chr_stop)[smart.id], base.ref = "N", 
                        ...
                        depth.tumor = c(varscan.copynumber$tumor_depth, 
                                        varscan.copynumber$tumor_depth)[smart.id], depth.ratio = c(varscan.copynumber$log2_ratio, 
                                                                                                   varscan.copynumber$log2_ratio)[smart.id], Af = 1, 
                        Bf = 0, zygosity.normal = "hom", GC.percent = c(varscan.copynumber$gc_content, 
                                                                        varscan.copynumber$gc_content)[smart.id], stringsAsFactors = FALSE)
